...
public class PutCallTest extends Base{
	//create a user
	//get the user
	//put call - updating the user details
	//get call 
		...
		System.out.println("---------------------------------------------------------------------------");
				...
				userdetails.setName("Jamuna");
				userdetails.setGender("female");
				userdetails.setStatus("inactive");
				
		
		System.out.println("Update the newly created user details");
		
			Response putresponse = client.put(GOREST_BASE_URI, GOREST_ENDPOINT.concat("/"+Userid), userdetails, null, AuthType.GOREST_BEARER_TOKEN, ContentType.JSON);
			
				AssertJUnit.assertEquals(putresponse.statusCode(), 200);
				AssertJUnit.assertTrue(putresponse.statusLine().contains("OK"));
				
				System.out.println("Updated Username : "+ putresponse.jsonPath().getString("name"));
				System.out.println("Updated Gender : "+ putresponse.jsonPath().getString("gender"));
				System.out.println("Updated Status : "+ putresponse.jsonPath().getString("status"));
				System.out.println("Updated email : "+ putresponse.jsonPath().getString("email"));
				
	System.out.println("---------------------------------------------------------------------------");	
	
	System.out.println("Get the updated user details of newly created user");	
